...
#define WDT_COUNT1_MATCH    (0x012Cu)
...
uint8 CustData[CUST_DATA_LEN] = {'w', 'n', 'd', 'w'};
...
{   
	...
	for(;;)
    {
        ...
        else if (1u == GPIO1flag)
        {
            ...
            //Set b1 to Proximity status
            //doing it this way so app server can enable/disable prox functions
            //PIRResult 0 = proximity detected, 1 = no prox detected
            //PIRResult = PollPIR()^1; //app-server logic needs determined still
            //CustData[STATUS_INDEX] |= PIRResult<<1;
            //CustData[STATUS_INDEX] &= PIRResult<<1;
            
            //set b0 to door open status (1)
            CustData[STATUS_INDEX] |= 0x01;
            
            //Swap window switch voltages
            Pin_1_Write(0u);
            Pin_2_Write(1u);
        ...
        {
            //set b0 to window closed status (0)
            CustData[STATUS_INDEX] &= ~0x01;
            
            //reset proximity readings to 0 (might make ole's work easier)
            //CustData[STATUS_INDEX] &= 0x01;
...
uint8 PollPIR()
{
    ...
    //reinit wdt, and re-disable isr
    WDTInit(WDT_COUNT0_MATCH, WDT_COUNT1_MATCH);
    WdtIsr_Disable();
    
    result = Pin_Pir_Vout_Read();
    
    //Pin_Pir_Vin_Write(0u);
